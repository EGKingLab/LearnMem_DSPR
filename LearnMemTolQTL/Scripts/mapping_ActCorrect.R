...
#mod <- lme(Learning ~ Activity_score, random = ~1|patRILF, data=rawdLM)
mod <- lm(Learning ~ Activity_score, data=rawdLM)
summary(mod)
rawdLM$LearnAC <- resid(mod)

#mod <- lme(Memory ~ Activity_score, data=rawdLM)
mod <- lm(Memory ~ Activity_score, data=rawdLM)
summary(mod)
...
pp<-myGenos[['phenotype']][,c('LearnPowerTrans','Memory_Mean','LearnAC_Mean', 'MemAC_Mean', 'Act_Mean')]
...
save(obs.LL, file="../ProcessedData/ActCorr_LODscores.rda")
...
ggsave(pall, filename = "../Plots/LM_correct_QTL.pdf", height=6.5,width=6.5)
